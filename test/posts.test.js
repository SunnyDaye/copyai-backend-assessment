...
const elementsAreUnique = require('./utils/elementsAreUnique');
...
describe('GET /api/posts', () => {
    ...
    it('Should return an error message with status code 400 if sortBy query parameter is invalid', async () => {
        //request
        const response = await request(app).get('/api/posts?tags=health,tech&sortBy=time');
        expect(response.body).to.have.property('error');
        expect(response.status).to.equal(400);
    ...
    it('Should return an error message with status code 400 if direction query parameter is invalid', async () => {
        //request
        const response = await request(app).get('/api/posts?tags=health,tech&direction=down');
        expect(response.body).to.have.property('error');
        expect(response.status).to.equal(400);
    });
});